<div>
    ...
    <div class="mb-8">
        <h2 class="text-2xl font-bold text-gray-800 tracking-tight">Withdrawal Management</h2>
        <p class="text-gray-500 text-sm">Reviewing all coin requests and taka payouts.</p>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y text-gray-600 text-sm">
                    ...
                    <tr wire:key="withdraw-{{ $w->id }}" class="hover:bg-gray-50 transition">
                        <td class="p-4">
                            <div class="font-bold text-gray-800">{{ $w->serial_no }}</div>
                            <div class="text-[10px] text-gray-400">{{ $w->created_at->format('d M Y, h:i A') }}</div>
                        </td>
                        <td class="p-4">
                            <div class="font-semibold text-blue-600">{{ $w->username }}</div>
                            <div class="text-[10px] text-gray-500 font-mono">{{ $w->acct_no }}</div>
                            <div class="text-[10px] text-gray-400 truncate w-48 mt-0.5">
                                Wallet: <span class="font-mono">{{ $w->wallet_address }}</span>
                            </div>
                        </td>
                        <td class="p-4">
                            <div class="text-xs">Req: <span class="font-bold text-gray-700">{{ number_format($w->request_amount_coin, 4) }}</span></div>
                            <div class="text-[10px] text-red-400">Charge: {{ number_format($w->charge_coin, 4) }} ({{ $w->pondage_percent }}%)</div>
                        </td>
                        ...
                        <td class="p-4 text-center">
                            <span class="px-3 py-1 rounded-full text-[10px] font-black uppercase 
                                ...
                                {{ $w->status == 'canceled' ? 'bg-red-100 text-red-700' : '' }}">
                                {{ $w->status }}
                            </span>
                        </td>
                        <td class="p-4">
                            <div class="flex justify-center gap-2">
                                @if($w->status == 'pending')
                                    <button wire:click="approve({{ $w->id }})" wire:confirm="Confirm Approval?" 
                                            class="bg-green-500 hover:bg-green-600 text-white p-2 rounded-lg shadow-sm transition">
                                        <i data-lucide="check" class="w-4 h-4"></i>
                                    </button>
                                    <button wire:click="cancel({{ $w->id }})" wire:confirm="Confirm Cancellation?" 
                    ...
                    @empty
                    <tr><td colspan="6" class="p-10 text-center text-gray-400">No records found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
